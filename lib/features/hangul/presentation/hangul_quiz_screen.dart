...
class _HangulQuizScreenState extends State<HangulQuizScreen> {
  ...
  Future<void> _speakIfEnabled(HangulCard question, String text) async {
    ...
    await _services.audioService.play(
      _promptCueFor(question, text, _questionIndex),
    );
  }
  ...
  PromptCue _promptCueFor(
    ...
    return PromptCue(
      AudioCueRef(
        assetPath:
            'assets/generated/audio/voice/prompts/hangul/${widget.lessonId}_quiz_$slug.mp3',
  ...
  Future<void> _selectChoice({
    ...
    if (isLastQuestion) {
      ...
      if (settings.effectsEnabled && earnedSticker) {
        try {
          await services.audioService.play(
            const RewardCue(AudioPackId('hangul')),
          );
        } catch (_) {}
        if (!mounted) {
          return;
