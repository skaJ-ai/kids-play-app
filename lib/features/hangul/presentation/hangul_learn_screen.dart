...
class _HangulLearnScreenState extends State<HangulLearnScreen> {
  ...
  Future<HangulLesson> _loadLesson() async {
    final lesson = await (widget.repository ?? HangulLessonRepository())
    ...
    final savedIndex = snapshot
        .progressFor('hangul:${widget.lessonId}')
        .lastViewedIndex;
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          flex: 5,
                          child: ToyPanel(
                            key: const ValueKey('hangulLearnSymbolPanel'),
